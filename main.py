...
def plot_train_acc(train_acc_list, epochs, name):
    plt.plot(range(1, epochs+1), train_acc_list)
    plt.title('Training Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.savefig(os.path.join('plots',name+'_plot_train_acc.png'))
    plt.clf()

def plot_train_loss(train_loss_list, epochs, name):
    plt.plot(range(1, epochs+1), train_loss_list)
    plt.title('Training Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.savefig(os.path.join('plots',name+'_plot_train_loss.png'))
    plt.clf()

def plot_test_acc(test_acc_list, epochs, name):
    plt.plot(range(1, epochs+1), test_acc_list)
    plt.title('Testing Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.savefig(os.path.join('plots',name+'_plot_test_acc.png'))
    plt.clf()
...
#     print(model)
    ...
    with open(os.path.join('results',args.expname+'_lists.pkl'), 'wb') as f:
        pickle.dump((train_acc_list, train_loss_list, test_acc_list), f)
#     with open('lists.pkl', 'rb') as f:
#         train_acc_list, train_loss_list, test_acc_list = pickle.load(f)
    
    plot_train_acc(train_acc_list, args.num_epochs, args.expname)
    plot_train_loss(train_loss_list, args.num_epochs, args.expname)
    plot_test_acc(test_acc_list, args.num_epochs, args.expname)